(function($) {
	...
	//Mirko edit Event: Touch swipeleft but needs jquery mobile
	...
	$('.show-modal').on('click', function () {
   	// 	event.preventDefault(); // cancel the event
			$.showYtVideo({
					videoId: $(this).attr('videoId') //'uVdV-lxRPFo'
			});
	});
	...
	$(window).on("navigate", function (event, data) {
	  ...
	  if (direction == 'back') {
		if (detailed>0)
		{
			...
			$('div.full').hide();
			$('html, body').animate({
			scrollTop: $('article.openpost[data-id="'+id+'"]').offset().top-20},10);
			detailed=-1;
	...
	$('a.openpost').click(function(){
		...
		history.pushState({}, '', 'expand='+id+'html');
	});
